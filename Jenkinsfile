                        ...
                        script {
                            ...
                            javaServices.each { service ->
                                dir(service) {
                                    ...
                                    try {
                                        sh "mvn org.sonarsource.scanner.maven:sonar-maven-plugin:3.10.0.2594:sonar -D sonar.projectKey=${service} -D sonar.host.url=http://sonarqube:9000 -D sonar.login=admin -D sonar.password=admin"
                                    } catch (Exception e) {
                    ...
                    steps {
                        script {
                             ...
                             pythonServices.each { service ->
                                dir(service) {
                                     ...
                                     try {
                                         ...
                                         sh "npx -y sonar-scanner -D sonar.projectKey=${service} -D sonar.sources=. -D sonar.host.url=http://sonarqube:9000 -D sonar.login=admin -D sonar.password=admin"
                                     } catch (Exception e) {
                                         echo "SonarQube analysis failed for ${service}: ${e.message}"
                                     }
                                }
                            ...
                            dir('secflowcheck-frontend-v2') {
                                ...
                                try {
                                    sh 'npx -y sonar-scanner -D sonar.projectKey=secflowcheck-frontend-v2 -D sonar.sources=src -D sonar.host.url=http://sonarqube:9000 -D sonar.login=admin -D sonar.password=admin'
                                } catch (Exception e) {
