                        ...
                        script {
                            ...
                            javaServices.each { service ->
                                dir(service) {
                                    ...
                                    try {
                                        sh "mvn org.sonarsource.scanner.maven:sonar-maven-plugin:3.10.0.2594:sonar -Dsonar.projectKey=${service} -Dsonar.host.url=http://localhost:9000 -Dsonar.login=admin -Dsonar.password=admin"
                                    } catch (Exception e) {
                    ...
                    steps {
                        script {
                             ...
                             pythonServices.each { service ->
                                dir(service) {
                                     ...
                                     try {
                                         ...
                                         sh "npx -y sonar-scanner -Dsonar.projectKey=${service} -Dsonar.sources=. -Dsonar.host.url=http://localhost:9000 -Dsonar.login=admin -Dsonar.password=admin"
                                     } catch (Exception e) {
                                         echo "SonarQube analysis failed for ${service}: ${e.message}"
                                     }
                                }
                            ...
                            dir('secflowcheck-frontend-v2') {
                                ...
                                try {
                                    sh 'npx -y sonar-scanner -Dsonar.projectKey=secflowcheck-frontend-v2 -Dsonar.sources=src -Dsonar.host.url=http://localhost:9000 -Dsonar.login=admin -Dsonar.password=admin'
                                } catch (Exception e) {
